...
async function startServer() {
  ...
  // Migrate existing tables if they don't have the referred_by column
  try {
    await db.exec(`ALTER TABLE responses ADD COLUMN referred_by TEXT;`);
  } catch (error) {
    ...
    try {
      const result = await db.run(
        `INSERT INTO responses (
          age, site, site_other, frequency, referred_by,
        ...
        [
          data.age, data.site, data.siteOther, data.frequency, data.referredBy || null,
